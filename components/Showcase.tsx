    ...
    <section id="showcase" className="relative py-24 overflow-hidden bg-background">
        ...
        <div className="text-center mb-16">
          <Reveal>
            ...
            <h2 className="text-3xl md:text-5xl font-serif font-black text-slate-900 dark:text-white mb-6">
              Une expérience <span className="text-primary italic">fluide.</span>
            </h2>
            <p className="text-lg text-slate-600 dark:text-slate-400 font-normal max-w-2xl mx-auto">
              Chaque écran a été conçu pour simplifier le quotidien scolaire et favoriser la sérénité.
            </p>
          </Reveal>
        </div>
      </div>
      ...
      <div className="relative group/showcase">
        <div className="absolute top-1/2 -translate-y-1/2 left-0 right-0 z-20 flex justify-between px-6 lg:px-12 pointer-events-none hidden lg:flex">
          <button 
            onClick={() => scroll('left')}
            className={`p-4 rounded-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 transition-all pointer-events-auto shadow-xl ${
              canScrollLeft ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4'
            }`}
          >
          ...
          <button 
            ...
            className={`p-4 rounded-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 transition-all pointer-events-auto shadow-xl ${
              canScrollRight ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-4'
            }`}
          >
                  ...
                  <div className="w-full h-full rounded-[2.5rem] overflow-hidden bg-white dark:bg-slate-900 relative flex flex-col">
                    ...
                    <div className="flex-1 px-6 space-y-5">
                      <div className="flex items-center justify-between">
                         ...
                         <span className="text-[8px] bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full text-slate-500 font-bold">ACTIF</span>
                      </div>
